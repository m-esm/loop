...
import { isActiveStatus, type MessageSnapshot, type RoomFile, type RoomFilesSnapshot, type TaskSnapshot } from '@loop/types';
...
export default function Room() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      api<RoomFilesSnapshot>('/rooms/default/files', { signal: controller.signal }),
  ...
  return <>
    ...
    <nav aria-label="Room views">
      ...
      {cursors && me && <TasksFeed since={Math.min(cursors.tasks, cursors.messages)} onUnauthorized={signedOut} onEvent={(event) => {
        ...
        if (event.room_id === 'default' && event.id > baseline) {
          setState((rows) => applyTaskEvent(rows, event));
          if (event.kind === 'message_created' && event.payload.message.body.kind === 'file') {
            void api<RoomFilesSnapshot>('/rooms/default/files').then((snapshot) => setFiles(snapshot.files)).catch(() => { /* next event retries */ });
          }
        }
      }} />}
    </nav>
    ...
    </section> : view === 'tasks' ? <TasksPanel tasks={state.tasks} />
      : <FilesPanel room="default" files={files} onChange={() => {
        void api<RoomFilesSnapshot>('/rooms/default/files').then((snapshot) => setFiles(snapshot.files)).catch((err: Error) => setError(err.message));
      }} />)}
  </>;
}
