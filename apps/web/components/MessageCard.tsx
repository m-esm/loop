...
export default function MessageCard({ message, task, author, tasks = [] }: {
  message: Message; task?: Task; author: string; tasks?: Task[];
}) {
  return <article className="message-card" data-message-id={message.id}>
    ...
    {message.body.kind === 'text' ? <p className="message-body">{message.body.text}</p>
      : task ? <TaskCard task={task} author={author} tasks={tasks} /> : <p>Loading task...</p>}
  </article>;
}
