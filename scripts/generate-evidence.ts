...
function generate(): string {
  ...
  const lines = [
    ...
    'Room verbosity controls stdout and stderr progress: Quiet suppresses task_progress writes, Normal keeps the existing 500ms coalescing, and Verbose flushes every output line. Changes apply to subsequent output in running turns. Exit 0 finishes `done` with the last nonempty stdout line; nonzero fails with the last nonempty stderr line in every mode.',
    'Migration `0016_room_verbosity.sql` adds checked, non-null `rooms.verbosity` defaulting to `normal`. Room summaries expose it; only owners can PATCH steering with `quiet`, `normal`, or `verbose`. The runner passes the mode at spawn in `LOOP_TASK_VERBOSITY`. The main composer keeps the persisted dial in the steering panel beside Pause and Wrap up, disabled for members and omitted from thread composers.',
    ...
    'Migration `0015_room_steering.sql` adds nullable `rooms.paused_at` and boolean `rooms.wrap_up`. Room reads return `paused` and `wrapUp`. Only owners can `PATCH /rooms/:id/steer` with boolean `paused` or `wrapUp` fields. Pause survives restart, blocks new claims, and lets existing turns finish. Resume allows queued tasks to run on the next runner tick. Wrap up stays queued while paused or idle, then the next runnable agent turn consumes it and receives the converge instruction in `LOOP_TASK_STEERING`. Agent commands must read that environment variable alongside the existing task inputs. These controls live in a panel behind the main composer\'s Steering toggle, collapsed at rest and reporting paused and wrap-up state on the toggle itself; they refresh every two seconds and on focus, and thread composers omit them.',
